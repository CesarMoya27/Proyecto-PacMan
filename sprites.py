...
class Jugador:
    ...
    def respawn(self):
        ...
        self.rect.center=(self.x, self.y)
        self.can_move=False
        self.respawn_timer=PLAYER_RESPAWN_TIME/1000.0
    ...
    def handle_input(self, dt):
        ...
        if self.can_move:
            ...
            if keys[pygame.K_RIGHT]:
                self.dx=PLAYER_SPEED*dt*60
            elif keys[pygame.K_LEFT]:
                self.dx=-PLAYER_SPEED*dt*60
            elif keys[pygame.K_DOWN]:
                self.dy=PLAYER_SPEED*dt*60
...
class Ghost:
    ...
    def hide(self):
        ...
        self.respawn_timer=GHOST_RESPAWN_TIME/1000.0
        ...
        if self.can_move:
            if not self.vulnerable:
                if self.direction==RIGHT:
                    self.dx=GHOST_SPEED*dt*60
                elif self.direction==LEFT:
                    self.dx=-GHOST_SPEED*dt*60
                elif self.direction==UP:
                    self.dy=-GHOST_SPEED*dt*60
                elif self.direction==DOWN:
                    self.dy=GHOST_SPEED*dt*60
            else:
                if self.direction==RIGHT:
                    self.dx=GHOST_SPEED_V*dt*60
                elif self.direction==LEFT:
                    self.dx=-GHOST_SPEED_V*dt*60
                elif self.direction==UP:
                    self.dy=-GHOST_SPEED_V*dt*60
                elif self.direction==DOWN:
                    self.dy=GHOST_SPEED_V*dt*60
    ...
    def update(self, walls, dt):
        if not self.vulnerable:
            self.image=self.animacion_normal[int(self.indice_frame)]
        else:
            self.image=self.animacion_vulnerable[int(self.indice_frame)]
